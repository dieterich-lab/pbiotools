...
def read_bitseq_alphas(filename, header=None, comment="#", sep=" "):
    ...
    bitseq_alphas = pd.read_csv(
        filename.strip(), header=header, comment=comment, sep=sep
    )
    new_cols = []
...
def read_bitseq_means(
    filename, names=["mean", "variance"], comment="#", header=None, sep=" "
):
    ...
    mean_df = pd.read_csv(
        filename.strip(), comment=comment, names=names, header=header, sep=sep
    )
    return mean_df


def read_bitseq_tr_file(
    filename,
    names=["source_name", "transcript_id", "length", "effective_length"],
    header=None,
    comment="#",
    sep=" ",
):
    ...
    transcript_info = pd.read_csv(
        filename.strip(), names=names, header=header, comment=comment, sep=sep
    )
...
def read_idmapping_selected_file(idmapping_selected_file):
    ...
    idmapping_selected = pd.read_csv(
        idmapping_selected_file, sep="\t", header=None, names=IDMAPPING_SELECTED_NAMES
    )
...
def read_gaf_file(gaf_file):
    ...
    gaf = pd.read_csv(gaf_file, sep="\t", header=None, comment="!", names=GAF_NAMES)

    return gaf


EC2GO_NAMES = ["EC", "GO_description", "GO"]
...
def read_ec2go_file(ec2go_file):
    ...
    converters = {"EC": str.strip, "GO_description": str.strip, "GO": str.strip}

    ec2go_df = pd.read_csv(
        ec2go_file,
        sep=">|;",
        comment="!",
        engine="python",
...
def read_ncbi_taxonomy_nodes_file(nodes_file):
    ...
    nodes_df = pd.read_csv(
        nodes_file, sep="\t\|\t", header=None, names=NODES_NAMES, engine="python"
    )
    return nodes_df
...
def get_bowtie2_index_files(base_index_name):
    ...
    utils.raise_deprecation_warning(
        "misc.utils.bio.get_bowtie2_index_files", "misc.utils.pgrm_utils"
    )
    ...
    bowtie_files = [
        "{}{}".format(base_index_name, bowtie_extension)
